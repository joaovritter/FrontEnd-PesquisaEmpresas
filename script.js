...
function construirQuery(logradouro, numero, cidade, estado) {
    ...
    const arrayPesquisa = [logradouroExpandido, numero, cidade, estado]
    const arrayPesquisaFiltrado = arrayPesquisa.filter((item) => !!item)
    const pesquisa = arrayPesquisaFiltrado
        .map((item) => `'${item}'`)
        .join(" & ")
    return {
        expressao: pesquisa,
...
async function enviarQuery(query) {
    try {
        ...
        a.href = downloadUrl
        a.download = "resultados.xlsx" // Set desired file name
